node {
   ...
   stage('Preparation') {
       git 'https://github.com/joseht88/ms-zuul-gateway.git'
       mvnHome = tool 'MAVEN'
   }
   ...
   stage('Deployment') {
       try {
			sh 'chown jenkins:jenkins /var/lib/jenkins/workspace/ms-zuul-gateway/runDeployment.sh'
			sh 'chmod +x /var/lib/jenkins/workspace/ms-zuul-gateway/runDeployment.sh'
            sh '/var/lib/jenkins/workspace/ms-zuul-gateway/runDeployment.sh'
       }catch (e){
